...
export async function generateMetadata({ params }: ProjectPageProps): Promise<Metadata> {
  ...
  const image =
    mediaImageUrl(project.meta?.image) ?? mediaImageUrl(project.coverImage) ?? (await getDefaultOgImage())
...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return (
    <article className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
      <header>
        <p className="text-accent text-xs font-medium tracking-[0.25em] uppercase sm:text-sm">
          {t(`categories.${project.category}`)}
        </p>
        <h1 className="mt-3 text-3xl font-bold tracking-tighter text-balance sm:text-5xl">
          {project.title}
        </h1>
