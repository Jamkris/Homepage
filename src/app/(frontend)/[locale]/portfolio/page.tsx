  ...
  const activityItems: TimelineItem[] = activitiesResult.docs
    ...
    .map((activity) => {
      ...
      return {
        ...
        year: yearOf(activity.startDate),
    ...
    <div className="mx-auto max-w-5xl px-4 py-14 sm:px-6 sm:py-20">
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-16 sm:mt-20">
          {items.map((item, i) => (
            <Reveal key={item.key} delay={Math.min(i * 0.04, 0.3)}>
              <article className="border-border grid grid-cols-1 gap-5 border-t py-8 sm:grid-cols-[1fr_auto] sm:gap-10 sm:py-10">
